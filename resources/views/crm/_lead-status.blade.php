...
<div class="panel">
    ...
    <div style="padding:16px 20px;display:flex;flex-direction:column;gap:14px;">
        ...
        @if ($lead->is_locked && $lead->conversion_status === 'pending_approval')
            ...
            @endif
        @elseif ($lead->conversion_status === 'approved')
        ...
        @elseif ($lead->canEdit(auth()->user()))
            <form method="POST" action="{{ route("$routePrefix.status", $lead) }}" style="display:flex;gap:10px;">
                @csrf
                ...
                <button type="submit" class="btn btn-ghost" style="padding:9px 14px;font-size:12.5px;white-space:nowrap;">Update</button>
            </form>
        @endif
    </div>
</div>
