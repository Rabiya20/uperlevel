...
@section('title', 'Edit Role — UperLevel')
...
<form method="POST" action="{{ route('admin.company.roles.update', $role) }}">
    @csrf
    @method('PUT')
    @include('admin.company.roles._form', [
        'role' => $role, 'adminModules' => $adminModules, 'employeeModules' => $employeeModules, 'permissions' => $permissions,
    ])
</form>
@endsection
