@php
    $s = fn ($field, $default = '') => old($field, $component->{$field} ?? $default);
@endphp

<div class="panel" style="max-width:480px;">
    <div class="panel-head"><h3>Component Details</h3></div>
    <div style="padding:18px 20px;display:flex;flex-direction:column;gap:14px;">
        <div>
            <label class="f-label">Name</label>
            <input class="f-input" type="text" name="name" value="{{ $s('name') }}" maxlength="80" placeholder="e.g. Basic, Fuel, Tax" required>
        </div>
        <div>
            <label class="f-label">Type</label>
            <select class="f-input" name="type" required>
                <option value="earning" @selected($s('type', 'earning') === 'earning')>Payable (earning)</option>
                <option value="deduction" @selected($s('type') === 'deduction')>Deductible</option>
            </select>
        </div>
        <label class="f-check">
